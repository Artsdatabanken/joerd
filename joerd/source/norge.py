...
class norge(object):
    ...
    def __init__(self, options={}):
        self.base_dir = options.get('base_dir', 'norge')
        self.norge_url = options['url']
        self.download_options = options
        self.target_name = 'norge_16_epsg3857.tif'

    def get_index(self):
        # norge needs no index - it's a single file, for which we'll need
    ...
    def freeze_dry(self):
        # there's only one norge tile
        return dict(type='norge')

    def rehydrate(self, data):
        assert data.get('type') == 'norge', \
            "Unable to rehydrate %r from norge." % data
        return self
